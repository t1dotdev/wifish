...
<img src="public/wifish-logo.svg" width="96" height="96" alt="wifish logo" />
...
- **Durable runs** — a crack spawns detached and writes straight to its log file, so page refreshes and disconnects leave the process running; the session is restored on reload. One panel-started crack at a time.
...
Plus Node.js (v22+ recommended — the tests rely on native TypeScript type stripping).
...
Operator data lives under `.wifish/` in the repo root (gitignored):
...
It's built to stay local: single operator, offline by nature, all file access confined to the four data directories through a basename guard (no path traversal), and no data leaves your machine.
